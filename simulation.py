...
def run_visualization_episode(agent, episode_num_vis, max_steps_vis=3600, sim_seed_offset=0):
    ...
    try:
        while traci.simulation.getMinExpectedNumber() > 0 and current_step < max_steps_vis:
            ...
            # Agent makes a decision based on state or minimum green time
            if not is_currently_green or phase_decision_timer >= MIN_GREEN_TIME:
                ...
                if current_tl_sumo_phase_idx != target_sumo_green_phase:
                    # Initiate yellow phase transition
                    if current_tl_sumo_phase_idx in GREEN_TO_YELLOW_PHASE:
                        ...
                        # Advance simulation for yellow time
                        for _ in range(int(YELLOW_TIME / traci.simulation.getDeltaT())):
                            if traci.simulation.getMinExpectedNumber() == 0: break # Stop if no more vehicles
                            traci.simulationStep(); current_step += 1
                        if traci.simulation.getMinExpectedNumber() == 0: break # Break outer loop if simulation ended
                        
                    # Switch to the target green phase
                ...
                # If agent chooses to stay in current green, phase_decision_timer continues counting.
            ...
            # Update state for next decision cycle
            current_sumo_state_for_eval = get_sumo_state_eval()
            ...
            # Check for end of simulation (no more vehicles)
    ...
    except traci.exceptions.FatalTraCIError as e: print(f"Fatal TraCI Error during visualization: {e}")
    except KeyboardInterrupt: print("Visualization interrupted by user.")
    except Exception as e:
        print(f"Unexpected Python error during visualization episode:")
        traceback.print_exc()
    finally:
        try:
