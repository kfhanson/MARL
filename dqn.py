import numpy as np
import random
from collections import deque # Not strictly needed for offline if loading all data at once
...
LEARNING_RATE = 0.0005 # Often good to use a slightly smaller LR for offline
GAMMA = 0.95
...
# This CSV should be the one generated by sumo_data_logger.py with 4-phase logic
DATA_CSV_FILE = "sumo_data.csv"

# --- DQNAgentOffline Class (Adapted for 4 actions) ---
class DQNAgentOffline:
    def __init__(self, state_dims, action_size, sequence_length):
        self.state_feature_size = state_dims
        self.action_size = action_size # This will be 4
    ...
    def _build_lstm_model(self):
        model = Sequential([
            ...
            LSTM(32, activation='relu', return_sequences=False), # LSTM units: 32, tune as needed
            # Dense(24, activation='relu'), # Optional intermediate layer
            Dense(self.action_size, activation='linear') # Output Q-values for each of the 4 actions
        ])
    ...
    def train_on_batch(self, batch_experiences):
        ...
        for exp_state, _, _, exp_next_state, _ in batch_experiences:
            ...
            current_states_batch_list.append(state_np[0]) # Get the (seq_len, features) part
        ...
        for i, (state_tuple, action, reward, next_state_tuple, done) in enumerate(batch_experiences):
            ...
            # Ensure action is an integer and within bounds for indexing
...
def load_offline_data_from_csv(csv_filepath):
    ...
    for _, row in df.iterrows():
        try:
            ...
            # Ensure action is valid for the 4-action setup (0, 1, 2, 3)
            if not (0 <= action < NUM_ACTIONS):
                # print(f"Warning: Skipping row with invalid action: {action}. Row: {row.to_dict()}")
                continue
            ...
            done_val = row['done']
            # Handle various ways 'done' might be stored (True/False, 1/0, "True"/"False")
...
def train_offline_agent(total_epochs=NUM_EPOCHS, model_save_path=MODEL_SAVE_PATH):
    ...
    # agent.load_model(model_save_path) # Uncomment to continue training a saved model
    ...
    for epoch in range(total_epochs):
        random.shuffle(experiences_dataset) # Shuffle data for each epoch
        ...
        if (epoch + 1) % 10 == 0 or epoch == total_epochs -1 : # Save periodically and at the end
